...
import { sendNotificationToAllUsers } from '../services/fcmService';
...
const NotificationsManagement = () => {
  ...
  const sendFCMNotification = async (notification) => {
    ...
    try {
      ...
      // Update notification with sent count, timestamp, and target audience
      await updateDoc(doc(db, 'notifications', notification.id), {
        ...
        targetAudience: 'all_users' // Update target to all users when sent
      });
  ...
  // Generate AI notification campaign (save + send to test token)
  const generateCampaign = async () => {
    try {
      setTestingNotification(true);
      setLastNotificationResult(null);
      
      toast.loading('Generating campaign...', { id: 'generate-campaign' });
      ...
      const response = await fetch('/api/ai-notification', {
        ...
        body: JSON.stringify({
          prompt: 'Create a compelling daily notification about eSIM deals and travel connectivity',
          saveOnly: true // Save to database
        })
      });
      ...
      if (result.success) {
        ...
        toast.success(`Campaign generated and test sent!`, { id: 'generate-campaign' });
      ...
      toast.error(`Error: ${error.message}`, { id: 'generate-campaign' });
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex gap-3">
            <button
              ...
              className="bg-orange-600 hover:bg-orange-700 disabled:bg-gray-400 text-white px-6 py-3 rounded-lg font-medium transition-colors flex items-center"
      ...
      {lastNotificationResult && (
        <div className={`bg-white rounded-xl shadow-lg p-6 mb-6 border-2 ${
          lastNotificationResult.error ? 'border-red-500' : 'border-green-500'
        }`}>
          <h3 className="text-lg font-semibold text-gray-900 mb-4">
            {lastNotificationResult.error ? '❌ Campaign Generation Failed' : 'Last Generated Campaign'}
          </h3>
          {lastNotificationResult.error ? (
            <div className="bg-red-50 p-4 rounded-lg">
              <p className="text-red-800 font-medium">{lastNotificationResult.error}</p>
            </div>
          ) : (
            <div className="space-y-3">
              <div className="bg-green-50 p-4 rounded-lg">
                <p className="text-sm text-gray-700 mb-2">
                  <strong>Generated Message:</strong> {lastNotificationResult.generated?.body}
                </p>
                <p className="text-sm text-gray-700">
                  <strong>Title:</strong> {lastNotificationResult.generated?.title}
                </p>
              </div>
              <div className="bg-blue-50 p-4 rounded-lg">
                <p className="text-sm text-blue-800 font-medium">
                  <strong>Status:</strong> Campaign generated successfully
                </p>
              </div>
              <p className="text-xs text-gray-500 text-right">
                Generated at: {lastNotificationResult.timestamp ? new Date(lastNotificationResult.timestamp).toLocaleString() : 'Just now'}
              </p>
            </div>
          )}
        </div>
