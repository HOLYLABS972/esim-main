...
export default async function sitemap() {
  ...
  // Country travel guide pages (generated by scripts/generate-country-guides.js)
  const countryGuideEntries = [];
  try {
    const txt = fs.readFileSync(path.join(process.cwd(), 'app/countries.txt'), 'utf-8');
    const seen = new Set();
    for (const line of txt.split('\n')) {
      ...
      seen.add(name);
      const slug = 'travel-guide-' + name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
      countryGuideEntries.push({
    ...
    // countries.txt not available at build time — skip
  }

  return [...staticPages, ...blogEntries, ...countryGuideEntries];
}
